...
const SpinWheel = ({ prizes, onSpinStart, onSpinEnd }: SpinWheelProps) => {
    ...
    // Initialize audio context and result sounds
    useEffect(() => {
        try {
        ...
        // Initialize result sounds
        congratsAudioRef.current = new Audio('/sounds/winning.mp3')
        congratsAudioRef.current.volume = 0.6
        
        loseAudioRef.current = new Audio('/sounds/fail-unlucky.mp3')
        loseAudioRef.current.volume = 0.5

        return () => {
            if (congratsAudioRef.current) congratsAudioRef.current.pause()
    ...
    const playWinSound = () => {
        if (congratsAudioRef.current) {
            ...
            congratsAudioRef.current.play().catch(err => console.log('Congrats sound play failed:', err))
        }
    }
    ...
    const playLoseSound = () => {
        if (loseAudioRef.current) {
            ...
            loseAudioRef.current.play().catch(err => console.log('Lose sound play failed:', err))
        }
    }
